...
public class StandardOrderHandler extends OrderHandler {
    ...
    public boolean handleOrder(Order order) {
        if (order.getTotalPrice() < 100.0){
            order.setDiscoutn(0.05);
            System.out.println("Valor com desconto padrão: R$ "+new DecimalFormat("0.00").format(order.getTotalPrice() - order.getTotalPrice()*order.getDiscoutn()));
            return true;
        } else {
